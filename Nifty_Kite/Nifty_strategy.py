...
from Nifty_Options import get_dictionary_strike_price_instruments_min_expiry
from Trade_kite import get_strike_price_from_index_indicator, buy_contract, sell_contract, Trade

# market time is not checked if test run is true
test_run = False
google_sheet_enabled = True
#global variables for call put contracts
real_trade = True
call_contracts_dictionary = None
put_contracts_dictionary = None
quantity = 50
entry_strike_price = 0
contract_row = None
contract_expiry = None

# if not test_run and platform.system() != 'Darwin':
#     logging.basicConfig(filename='C:\\Users\\Administrator\\Downloads\\Nifty-project\\Task Scheduler Task Output logs\\output.log', level=logging.INFO)
...
from fileUtls import add_line_to_file, is_file_empty, getLogStr_from_dfRow, add_line_break_to_last_line, \
    write_to_pickle_file, read_from_pickle_file
...
from global_strings import time_s, open_s, high_s, low_s, close_s, volume_s, rsi_s, close_ma_s, volume_ma_s, gain_s, \
    loss_s, \
    avg_gain_s, avg_loss_s, rsi_ma_s, buy_sell_log_file
...
from global_strings import sheet_details_trade_log, sheet_details_candle_log, CLIENT_SECRET_FILE, API_SERVICE_NAME, \
    API_VERSION, SCOPES, token_file, running_trade_file
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        if tick["instrument_token"] != Nifty50_token:
            ...
            nifty_tick_in_ticks = True

    if nifty_tick_in_ticks:
...
def check_for_trade_entry():
    ...
    global stoploss, target, entry_index, entry_strike_price
    global nifty_tick, now, cumulative_profit, contract_row, contract_expiry
    ...
    if trade_entry:
        ...
        if in_trade:
            ...
            entry_strike_price = get_strike_price_from_index_indicator(entry_index, trade_type)
            contract_row = None
            if trade_type == Trade.LONG_POSITION:
                contract_row = call_contracts_dictionary.get(int(entry_strike_price))
            elif trade_type == Trade.SHORT_POSITION:
                contract_row = put_contracts_dictionary.get(int(entry_strike_price))
            print(contract_row)
            # contract_expiry = datetime.combine(contract_row["expiry"].date(), end_time.time())
            contract_expiry = datetime.combine(contract_row["expiry"], end_time.time())
            contract_expiry = contract_expiry - timedelta(minutes=candle_time)
            if now > contract_expiry:
                print("Long occured at time " +str(now)+ " but not placing order since contract will expire soon")
                return

            trade_placed, order_id, log_buy_sell = buy_contract(contract_row, quantity, real_trade, kite)

            add_line_to_file(buy_sell_log_file, log_buy_sell)
            send_telegram_message(log_buy_sell)
            if not trade_placed:
        ...
        write_to_pickle_file(running_trade_file, contract_row)
...
def check_for_trade_exit():
    ...
    global nifty_tick, now, contract_row
    ...
    if in_trade:
        if trade_type == Trade.LONG_POSITION:
            ...
            elif now > contract_expiry:
                trade_exit = True
                log_data_frame.loc[log_index, indicator] = "reached expiry"
    ...
    if trade_exit:
        ...
        if current_time <= start_time:
            # Sleep for 3 seconds
            time.sleep(3)
            print("Code slept for 2 seconds as the time is 9:15 am or earlier")
        else:
            log_data_frame.loc[log_index, time_s] = now
            log_data_frame.loc[log_index, index_s] = nifty_tick
            ...
            if contract_row is None:
                if trade_type == Trade.LONG_POSITION:
                    contract_row = call_contracts_dictionary.get(entry_strike_price)
                elif trade_type == Trade.SHORT_POSITION:
                    contract_row = put_contracts_dictionary.get(entry_strike_price)

            trade_placed, order_id, log_buy_sell = sell_contract(contract_row, quantity, real_trade, kite)
...
def main():
    ...
    global instruments_dict, instrument_tokens_nifty50, Nifty50_token, kite
    global live_data_frame, log_data_frame, service, contract_expiry, call_contracts_dictionary, put_contracts_dictionary
    global in_trade, trade_type, cumulative_profit, log_data_frame, entry_index, stoploss, target, entry_strike_price,\
        nifty_tick, now, contract_row
    ...
    if is_file_empty(trade_log_file):
        ...
        if temp_log_df.__len__() > 0:
            if temp_log_df[trade].iloc[-1] == "entry" or temp_log_df[trade].iloc[-1] == "reset tgt/sl":
                ...
                k = 0
                for k in reversed(temp_log_df.index):
                    entry_type = temp_log_df[trade].iloc[k]
                    if entry_type == "entry":
                        break;
                if temp_log_df[trade].iloc[k] == "reset tgt/sl":
                    k = -1

                entry_index = float(temp_log_df[index_s].iloc[k])
                entry_strike_price = get_strike_price_from_index_indicator(entry_index, trade_type)
    ...
    #get instruments and other values from NFO
    call_contracts_dictionary, put_contracts_dictionary = get_dictionary_strike_price_instruments_min_expiry(kite)

    contract_row = None
    if not is_file_empty(running_trade_file):
        contract_row = read_from_pickle_file(running_trade_file)

    if contract_row is None:
        contract_expiry_row = next(iter(call_contracts_dictionary.values()))
        contract_expiry = datetime.combine(contract_expiry_row["expiry"], end_time.time())
    else:
        contract_expiry = datetime.combine(contract_row["expiry"], end_time.time())
